    ...
    angular
        ...
        .directive('mdrFile', ['$compile', function($compile) {
            var linker = function(scope, element, attrs) {
                ...
                scope.internalControl.clearFiles = function() {
                    scope.model = [];
                    element.find('#fileId_' + scope.$id + ' .mdr-file-dad-content div').fadeOut('slow', function() {
                        $(this).remove();
                    });
                    element.find("#fileId_" + scope.$id + ' .mdr-file-dad-content button').fadeOut('slow');
                }

                // add event for button delete
                // $("#fileId_" + $scope.$id + ' .mdr-file-dad-content .preview-' + k
            };
        ...
        .controller('FileCtrl', ['$scope', '$element', '$attrs', function($scope, $element, $attrs) {
            ...
            */
            ...
            $scope.clearContent = function() {
                $('#fileId_' + $scope.$id + ' .mdr-file-dad-content div').fadeOut('slow', function() {
                    $(this).remove();
                });
            ...
            function uploadFile(k, v) {
                ...
                function transferComplete(e) {
                    ...
                    if (xhr && xhr.status == 200) {
                        ...
                        console.log("Server error " + (xhr ? xhr.status : "") + ", an error occurred while transferring the file.");
                    }
                }
            ...
            function createPreview(v, k, url, icon, messages) {
                ...
                var preview =
                    ...
                    '</div>' +
                    '<div class="caption">' +
                    '<p class="wraptext">' + v.name + '</p>' +
                    '<p><span class="text-muted">' + humanSize + '</span></p>' +
                    '<button ng-click="clearContent()"><i class="fa fa-trash-o fa-fw"></i></button>' +
                    '</div>' +
                    '</div>' +
                    '</div>';
